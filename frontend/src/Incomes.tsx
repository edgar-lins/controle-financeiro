...
import type { Income, Toast as ToastState } from "./types";
...
export default function Incomes() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; id: number | null; desc: string }>({ isOpen: false, id: null, desc: "" });
  ...
  return (
    <div className="space-y-8 animate-fade-in relative z-10">
      ...
      <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-8">
        <div>
          <h2 className="font-headline text-3xl md:text-4xl font-extrabold tracking-tight mb-1 text-white">Renda Extra</h2>
          <p className="text-secondary/60 text-sm mb-4">Freelance, bônus, 13º e outras entradas além do salário fixo.</p>
          <div className="flex gap-8 mt-2">
            <div className="flex flex-col">
              <span className="text-secondary text-xs uppercase tracking-widest font-semibold mb-1 opacity-60">Total Extras</span>
      ...
      <section className="bg-surface-container-low/60 backdrop-blur-md rounded-3xl overflow-hidden border border-outline-variant/10 shadow-xl">
        <div className="px-6 md:px-8 py-6 flex justify-between items-center border-b border-outline-variant/10">
          ...
          <button onClick={() => openModal()} className="hidden md:flex bg-primary text-on-primary font-bold py-2 px-4 rounded-full items-center gap-1 shadow-[0_0_15px_rgba(90,240,179,0.2)] hover:scale-105 transition-transform">
            <span className="material-symbols-outlined text-sm">add</span> Adicionar Renda Extra
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
          ...
          <div className="relative bg-[#131b2e] border border-outline-variant/20 shadow-2xl rounded-[2rem] w-full max-w-lg p-6 md:p-8 animate-fade-in max-h-[90vh] overflow-y-auto no-scrollbar">
            <h3 className="font-headline text-2xl font-bold text-white mb-6">
              {editingId ? "Editar Renda Extra" : "Nova Renda Extra"}
            </h3>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] text-secondary font-bold uppercase tracking-wider mb-1 ml-1">Descrição</label>
                <input required type="text" placeholder="Ex: Freelance, Bônus, 13º..." className="w-full bg-surface-container-highest/40 border border-outline-variant/10 text-white rounded-xl p-3.5 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
              </div>
      ...
      <ConfirmModal isOpen={deleteModal.isOpen} title="Excluir Renda" message={`Deseja excluir "${deleteModal.desc}"?`} onConfirm={() => deleteModal.id !== null && deleteIncome(deleteModal.id)} onCancel={() => setDeleteModal({ isOpen: false, id: null, desc: "" })} confirmText="Excluir" isDangerous={true} />
