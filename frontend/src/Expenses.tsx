...
import type { Expense, Toast as ToastState, Summary, RecurringPending } from "./types";
...
export default function Expenses() {
  ...
  const [highlightId, setHighlightId] = useState<number | null>(incomingState?.highlightId ?? null);

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [toast, setToast] = useState<ToastState>({ show: false, message: "", type: "success" });
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; id: number | null; desc: string; groupId: number | null; installmentTotal: number }>({ isOpen: false, id: null, desc: "", groupId: null, installmentTotal: 1 });
  ...
  const [recurringPending, setRecurringPending] = useState<RecurringPending[]>([]);
  const [confirmingId, setConfirmingId] = useState<number | null>(null);

  const { refreshSummary } = useSummary();

  const [form, setForm] = useState({
    description: "", amount: "", category: "", group: "", payment_method: "", date: "", account_id: "", installments: "1", is_recurring: false,
  });
  ...
  async function fetchRecurringPending() {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/expenses/recurring-pending`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      setRecurringPending(Array.isArray(data) ? data : []);
    } catch { /* silencioso */ }
  }

  async function confirmRecurring(pending: RecurringPending) {
    setConfirmingId(pending.id);
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/expenses/confirm-recurring`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ source_id: pending.id, account_id: pending.account_id }),
      });
      if (res.ok) {
        setToast({ show: true, message: `"${pending.description}" confirmado!`, type: "success" });
        fetchExpenses();
        fetchRecurringPending();
        refreshSummary();
      } else {
        setToast({ show: true, message: "Erro ao confirmar despesa.", type: "error" });
      }
    } catch {
  ...
  async function deleteExpense(id: number, deleteGroup = false) {
    try {
      const token = localStorage.getItem("token");
  ...
  async function openModal(expense: Expense | null = null) {
    if (expense) {
      setEditingId(expense.id);
      const dateStr = expense.date ? expense.date.split('T')[0] : "";
      setForm({
        description: expense.description, amount: expense.amount.toString(), category: expense.category || "", group: expense.group || "", payment_method: expense.payment_method || "", date: dateStr, account_id: expense.account_id ? expense.account_id.toString() : "", installments: "1", is_recurring: expense.is_recurring,
      });
    } else {
      setEditingId(null);
      setForm({ description: "", amount: "", category: "", group: "", payment_method: "", date: new Date().toISOString().split("T")[0], account_id: "", installments: "1", is_recurring: false });
    }
    setIsModalOpen(true);

    // Busca o summary do mês atual para exibir o progresso do orçamento
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/summary`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) setModalSummary(await res.json());
    } catch { /* não-crítico — barra simplesmente não aparece */ }
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in relative z-10">
      ...
      {/* Gastos fixos pendentes de confirmação */}
      {recurringPending.length > 0 && (
        <section className="bg-amber-500/5 border border-amber-500/20 rounded-3xl p-5 space-y-3">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-amber-400 text-lg">pending_actions</span>
            <h3 className="font-headline font-bold text-sm text-amber-300 uppercase tracking-wider">
              Gastos fixos aguardando confirmação
            </h3>
            <span className="ml-auto text-xs bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full font-bold">{recurringPending.length}</span>
          </div>
          <div className="space-y-2">
            {recurringPending.map((p) => {
              const catDef = CATEGORIES.find(c => c.value === p.category);
              const CatIcon = catDef?.icon;
              return (
                <div key={p.id} className="flex items-center gap-3 bg-surface-container-low/60 rounded-2xl px-4 py-3">
                  <div className="w-8 h-8 rounded-xl bg-amber-500/10 flex items-center justify-center flex-shrink-0">
                    {CatIcon ? <CatIcon className="text-sm text-amber-400" /> : <span className="material-symbols-outlined text-sm text-amber-400">receipt</span>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-white truncate">{p.description}</p>
                    <p className="text-xs text-secondary/60">{formatCurrencyBR(p.amount)} · todo dia {p.recurrence_day ?? "?"}</p>
                  </div>
                  <button
                    onClick={() => confirmRecurring(p)}
                    disabled={confirmingId === p.id}
                    className="flex-shrink-0 px-4 py-1.5 bg-primary text-on-primary text-xs font-bold rounded-full hover:bg-primary/90 transition-all active:scale-95 disabled:opacity-50"
                  >
      ...
      <section className="bg-surface-container-low/60 backdrop-blur-md rounded-3xl overflow-hidden border border-outline-variant/10 shadow-xl">
        <div className="px-6 md:px-8 py-6 flex justify-between items-center border-b border-outline-variant/10">
          ...
          <button onClick={() => setIsQuickAddOpen(true)} className="hidden md:flex bg-primary text-on-primary font-bold py-2 px-4 rounded-full items-center gap-1 shadow-[0_0_15px_rgba(90,240,179,0.2)] hover:scale-105 transition-transform">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-outline-variant/5">
                {expenses.map((exp) => {
                  ...
                  const CatIcon = catDef?.icon;
                  const isGoalContribution = exp.description.startsWith("Aporte:");
                    ...
                    >
                      <td className="px-6 md:px-8 py-5">
                        <div className="flex items-center gap-3 md:gap-4">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isGoalContribution ? "bg-sky-400/10" : "bg-primary/10"}`}>
                            {isGoalContribution
                      ...
                      <td className="px-6 md:px-8 py-5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button onClick={() => setDeleteModal({ isOpen: true, id: exp.id, desc: exp.description, groupId: exp.installment_group_id ?? null, installmentTotal: exp.installment_total })} className="p-2 text-secondary hover:text-red-400 transition-colors"><span className="material-symbols-outlined text-xl">delete</span></button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {/* FAB Mobile Only */}
      <button onClick={() => setIsQuickAddOpen(true)} className="md:hidden fixed bottom-24 right-6 w-14 h-14 rounded-full bg-primary text-on-primary shadow-[0_8px_32px_rgba(90,240,179,0.4)] flex items-center justify-center active:scale-90 transition-transform z-50">
      ...
      {isQuickAddOpen && (
        <QuickAddExpense
          accounts={accounts}
          onSuccess={() => { fetchExpenses(); fetchInstallments(); refreshSummary(); }}
          onOpenFullForm={() => { setIsQuickAddOpen(false); openModal(); }}
          ...
          <div className="relative bg-[#131b2e] border border-outline-variant/20 shadow-2xl rounded-[2rem] w-full max-w-2xl animate-fade-in max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:divide-x md:divide-white/5">
                ...
                {/* Coluna direita — categoria + progresso do orçamento */}
                <div className="px-6 md:px-8 py-6 border-t border-white/5 md:border-t-0 space-y-4">
                  <div>
                    <label className="block text-[10px] text-secondary font-bold uppercase tracking-wider mb-3 ml-1">Categoria 50/30/20</label>
                    <CategorySelect value={form.category} group={form.group as Expense["group"]} onChange={({ category, group }) => setForm({ ...form, category, group })} />
                  </div>

                  {/* Barra de progresso do grupo selecionado */}
                  {modalSummary && form.group && (() => {
                    const groupMap = {
                      essencial: { real: modalSummary.real_fixos, ideal: modalSummary.ideal_fixos, label: "Essenciais", color: "bg-primary" },
                      lazer:     { real: modalSummary.real_lazer,  ideal: modalSummary.ideal_lazer,  label: "Estilo de Vida", color: "bg-amber-400" },
                      investimento: { real: modalSummary.real_invest, ideal: modalSummary.ideal_invest, label: "Investimento", color: "bg-sky-400" },
                    } as const;
                    const g = groupMap[form.group as keyof typeof groupMap];
                    if (!g || g.ideal === 0) return null;
                    const thisAmount = parseFloat(form.amount) || 0;
                    const projected = g.real + thisAmount;
                    const pct = Math.min((projected / g.ideal) * 100, 100);
                    const isOver = projected > g.ideal;
                    return (
                      <div className="rounded-2xl bg-surface-container-highest/30 border border-outline-variant/10 p-4 space-y-2">
                        <div className="flex justify-between items-baseline">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-secondary">{g.label} este mês</span>
                          <span className={`text-xs font-bold ${isOver ? "text-error" : "text-secondary/70"}`}>
                            {formatCurrencyBR(projected)} / {formatCurrencyBR(g.ideal)}
                          </span>
                        </div>
                        <div className="w-full h-2 bg-surface-container-highest rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-300 ${isOver ? "bg-error" : g.color}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                        <p className="text-[10px] text-secondary/50">
                          {isOver
                            ? `${formatCurrencyBR(projected - g.ideal)} acima do orçamento`
                            : `${formatCurrencyBR(g.ideal - projected)} disponível`}
                        </p>
                      </div>
                    );
                  })()}
                ...
                <div className="px-6 md:px-8 py-3 border-t border-white/5">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <div className="relative flex-shrink-0">
                      <input type="checkbox" className="sr-only" checked={form.is_recurring} onChange={(e) => setForm({ ...form, is_recurring: e.target.checked })} />
                      <div className={`block w-10 h-6 rounded-full transition-colors ${form.is_recurring ? 'bg-primary' : 'bg-surface-container-highest'}`}></div>
                      <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${form.is_recurring ? 'transform translate-x-4' : ''}`}></div>
                    </div>
                    <div>
                      <span className="text-sm font-semibold text-on-surface group-hover:text-white transition-colors">Gasto fixo mensal</span>
                      <p className="text-[10px] text-secondary/50 mt-0.5">Aparece todo mês aguardando confirmação</p>
                    </div>
                  </label>
                </div>
              )}
      ...
      {(() => {
        ...
        const groups: Record<number, Expense[]> = {};
        installmentExpenses.forEach(exp => {
          const gid = exp.installment_group_id ?? exp.id;
          if (!groups[gid]) groups[gid] = [];
          groups[gid].push(exp);
        });
        const activeGroups = Object.values(groups).filter((g: Expense[]) =>
          g.some((e: Expense) => new Date(e.date) >= today)
        );
        ...
        return (
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {activeGroups.map((group: Expense[]) => {
          ...
          <div className="relative bg-[#131b2e] border border-outline-variant/20 shadow-2xl rounded-[2rem] w-full max-w-sm p-7 animate-fade-in">
            ...
            {deleteModal.installmentTotal > 1 ? (
              <div className="flex flex-col gap-3">
                <button onClick={() => deleteModal.id !== null && deleteExpense(deleteModal.id, false)} className="w-full py-3 rounded-xl font-bold text-white bg-surface-container-high hover:bg-surface-container-highest border border-outline-variant/10 transition-colors text-sm">
                  Excluir só esta parcela
                </button>
                <button onClick={() => deleteModal.id !== null && deleteExpense(deleteModal.id, true)} className="w-full py-3 rounded-xl font-bold text-white bg-red-600 hover:bg-red-500 transition-colors text-sm active:scale-95">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => deleteModal.id !== null && deleteExpense(deleteModal.id)} className="flex-1 py-3 rounded-xl font-bold text-white bg-red-600 hover:bg-red-500 transition-colors active:scale-95">Excluir</button>
              </div>
            )}
          </div>
        </div>
