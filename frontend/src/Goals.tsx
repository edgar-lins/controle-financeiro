...
export default function Goals() {
  ...
  const [addMoneyModal, setAddMoneyModal] = useState<{ isOpen: boolean; id: number | null; name: string; currentAmount: number }>({ isOpen: false, id: null, name: "", currentAmount: 0 });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!payload.deadline) { (payload as Partial<typeof payload>).deadline = undefined; }
  ...
  function openModal(goal: Goal | null = null) {
    if (goal) {
  ...
  function openAddMoneyModal(goal: Goal) {
