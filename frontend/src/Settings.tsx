...
import type { Toast as ToastState, UserPreferences } from "./types";
  ...
  async function fetchPreferences() {
    try {
      ...
      if (res.ok) {
        ...
        setPreferences(data);
        setSalaryInput(data.expected_monthly_income > 0 ? String(data.expected_monthly_income) : "");
      }
    } catch (error) {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/preferences`, {
        ...
        body: JSON.stringify({
          ...
          expected_monthly_income: parseFloat(salaryInput) || 0,
        }),
      });
      if (res.ok) {
        setPreferences(prev => ({ ...prev, expected_monthly_income: parseFloat(salaryInput) || 0 }));
        setToast({ show: true, message: "Salário atualizado com sucesso!", type: "success" });
      } else {
        setToast({ show: true, message: "Erro ao salvar salário.", type: "error" });
      }
    } catch {
  ...
  async function handleDeleteAccount() {
    try {
      ...
      if (res.ok) {
        ...
        setToast({ show: true, message: "Senha incorreta. Verifique e tente novamente.", type: "error" });
      } else {
    ...
    <div className="space-y-12 animate-fade-in relative z-10 pb-10">
      ...
      <section>
        <div className="flex items-center justify-between mb-6 px-2">
          <h3 className="text-lg font-headline font-bold text-on-surface">Salário Mensal Fixo</h3>
          <span className="text-xs text-primary font-bold uppercase tracking-widest bg-primary/10 px-2 py-0.5 rounded">Gratuito</span>
        </div>
        <div className="bg-surface-container-low/60 backdrop-blur-md rounded-3xl p-6 md:p-8 border border-outline-variant/10 shadow-lg">
          <p className="text-secondary/70 text-sm mb-5 leading-relaxed">
            Defina seu salário fixo aqui. Ele serve como base para calcular os orçamentos da regra 50/30/20.
            Use a tela de <strong className="text-on-surface">Renda Extra</strong> para registrar entradas eventuais como freelance, bônus e 13º.
          </p>
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              <label className="block text-[10px] font-bold uppercase tracking-wider text-secondary mb-2">Valor mensal líquido</label>
              <CurrencyInput
                value={salaryInput}
                onChange={setSalaryInput}
                className="w-full bg-surface-container-highest/40 border border-outline-variant/20 text-white rounded-xl px-4 py-3.5 focus:border-primary/50 focus:ring-1 focus:ring-primary/30 outline-none transition-all font-headline font-bold text-lg"
              />
            </div>
            <button
              onClick={handleSaveSalary}
              disabled={savingSalary}
              className="px-6 py-3.5 bg-primary text-on-primary font-bold rounded-xl hover:bg-primary/90 active:scale-95 transition-all disabled:opacity-50 whitespace-nowrap"
            >
              {savingSalary ? "Salvando..." : "Salvar"}
            </button>
          </div>
          {preferences.expected_monthly_income > 0 && (
            <p className="text-xs text-secondary/50 mt-3">
              Atual: <span className="text-primary font-semibold">
                {new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(preferences.expected_monthly_income)}
              </span> / mês
            </p>
          )}
        </div>
      ...
      {deleteModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-surface/80 backdrop-blur-sm" onClick={() => { setDeleteModal(false); setDeletePassword(""); setDeleteConfirmText(""); }}></div>
          <div className="relative bg-[#131b2e] border border-red-500/20 shadow-2xl rounded-[2rem] w-full max-w-md p-8 animate-fade-in">
            ...
            <input
              type="text"
              className="w-full bg-surface-container-highest/40 border border-red-500/20 text-white rounded-xl p-3.5 focus:border-red-400 focus:ring-1 focus:ring-red-400 outline-none transition-all mb-4"
              placeholder="CONFIRMAR"
              value={deleteConfirmText}
              onChange={(e) => setDeleteConfirmText(e.target.value)}
            />
            <p className="text-sm text-white mb-2">Confirme sua senha:</p>
            <input
              type="password"
            ...
            <div className="flex gap-3">
              <button onClick={() => { setDeleteModal(false); setDeletePassword(""); setDeleteConfirmText(""); }} className="flex-1 py-3.5 rounded-xl font-bold text-secondary bg-surface-container-high hover:bg-surface-container-highest transition-colors">
                Cancelar
              </button>
              <button
                onClick={handleDeleteAccount}
                disabled={deleteConfirmText !== "CONFIRMAR" || deletePassword === ""}
