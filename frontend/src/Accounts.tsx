...
import type { Account, Toast as ToastState } from "./types";
...
function getAccountBrand(name: string, type: string) {
...
export default function Accounts() {
  ...
  function openPayFaturaModal(card: Account) {
    setPayingCard(card);
  ...
  return (
    <div className="space-y-8 animate-fade-in relative z-10">
      ...
      <ConfirmModal isOpen={deleteModal.isOpen} title="Excluir Conta" message={`Atenção! Ao excluir a conta "${deleteModal.name}", TODAS as transações vinculadas a ela ficarão sem conta (órfãs) e seu patrimônio total será reduzido. Deseja mesmo continuar?`} onConfirm={() => deleteModal.id !== null && deleteAccount(deleteModal.id)} onCancel={() => setDeleteModal({ isOpen: false, id: null, name: "" })} confirmText="Excluir Conta" isDangerous={true} />
